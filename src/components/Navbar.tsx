...
const Navbar = () => {
  ...
  return <nav className="py-[17px] mx-0 rounded bg-baft-darkgray">
      <div className="container mx-auto flex justify-between items-center">
        ...
        {/* Mobile menu button */}
        <button className="md:hidden flex items-center" onClick={toggleMobileMenu} aria-label="Toggle mobile menu">
        ...
        <div className="hidden md:flex items-center space-x-8">
          <Link to="/" className={cn("transition-colors hover:text-baft-gold", isScrolled ? "text-baft-darkgray" : "text-white drop-shadow-sm")}>
            Home
          </Link>
          <Link to="/about" className={cn("transition-colors hover:text-baft-gold", isScrolled ? "text-baft-darkgray" : "text-white drop-shadow-sm")}>
            About Us
          </Link>
          ...
          <Link to="/get-involved" className={cn("transition-colors hover:text-baft-gold", isScrolled ? "text-baft-darkgray" : "text-white drop-shadow-sm")}>
            Get Involved
          </Link>
          <Link to="/get-involved#donate" className={cn("px-5 py-2 rounded transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5", isScrolled ? "bg-baft-maroon text-white hover:bg-baft-maroon/90" : "bg-baft-gold text-baft-darkgray hover:bg-baft-gold/90")}>
            Donate
          </Link>
        </div>
      </div>
      ...
      <div className={`md:hidden absolute w-full bg-white shadow-md transition-all duration-300 ${isMobileMenuOpen ? 'max-h-screen py-4' : 'max-h-0 overflow-hidden'}`}>
        <div className="container mx-auto flex flex-col space-y-4">
          <Link to="/" className="text-baft-darkgray hover:text-baft-maroon transition-colors px-4 py-2" onClick={() => setIsMobileMenuOpen(false)}>
            Home
          </Link>
          <Link to="/about" className="text-baft-darkgray hover:text-baft-maroon transition-colors px-4 py-2" onClick={() => setIsMobileMenuOpen(false)}>
            About Us
          </Link>
          ...
          <Link to="/get-involved" className="text-baft-darkgray hover:text-baft-maroon transition-colors px-4 py-2" onClick={() => setIsMobileMenuOpen(false)}>
            Get Involved
          </Link>
          <Link to="/get-involved#donate" className="mx-4 px-5 py-2 bg-baft-maroon text-white rounded text-center hover:bg-baft-maroon/90 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
            Donate
          </Link>
        </div>
      </div>
